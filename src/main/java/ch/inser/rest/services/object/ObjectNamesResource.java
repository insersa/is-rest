...
/**
 * Ressource pour accéder à une collection d'objets métier d'un type donné
 *
...
public class ObjectNamesResource extends AbstractObjectNamesResource {
    /**
     * Logger
     */
     ...
     */
    public ObjectNamesResource(ServletContext aContext, String aObjectName) {
    ...
    public Response getList(@ApiParam(value = "Security token", required = true) @HeaderParam("token") String aToken,
        ...
        try {
            ...
            Claims claims = RestUtil.getClaims(aToken, iObjectName);
            if (claims == null) {
            ...
            if (aFieldname != null) {
                return getFieldsRequest(jsonToVo(aQuery), aFieldname, loggedUser, claims,
            ...
            json.add(Constants.TOKEN, SecurityUtil.getToken(claims, RestUtil.getContextManager()));
     ...
     *
     * @param aToken
     *            json web token avec temps d'expiration
     * @param aQuery
    ...
    @Path("count")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getCount(@ApiParam(value = "Security token", required = true) @HeaderParam("token") String aToken,
            @ApiParam(value = "Filter criteria as an expression json", required = false) @QueryParam("query") String aQuery) {
        ...
        try {
            ...
            // Build the response
            JsonObjectBuilder json = Json.createObjectBuilder().add("count", nb).add(Constants.TOKEN,
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public final Response updateList(@ApiParam(value = "Security token", required = true) @HeaderParam("token") String aToken,
        ...
        try {
            ...
            // -- Contrôle si paramètre présent
            Claims claims = RestUtil.getClaims(aToken, iObjectName);
            if (claims == null || aRecords == null && aDeletes == null) {
                return Response.status(Status.BAD_REQUEST).build();
            }
            ...
            // Build the response
            JsonObjectBuilder json = Json.createObjectBuilder().add("nbr", result.getNbrRecords()).add("token",
                    SecurityUtil.getToken(claims, RestUtil.getContextManager()));
    ...
    public Response create(@ApiParam(value = "Security token", required = true) @HeaderParam("token") String aToken,
