...
/**
 * Service dynamique pour CRUD des objets métiers avec authentification OIDC
 *
...
@Path("/objects")
@Api(value = "objects")
public class ObjectsResourceOIDC {
     ...
     */
    @Path("{objectname}")
    public ObjectNamesResourceOIDC getObjectNamesResource(@PathParam("objectname") String aObjectName) throws ISException {
        String onrClassName = (String) getVOInfo(aObjectName).getValue(VOInfo.REST_ONR_CLASSNAME);
        if (onrClassName == null) {
            return new ObjectNamesResourceOIDC(iContext, aObjectName);
        }

        try {
            Class<?> cl = Class.forName(onrClassName);
            Constructor<?> constr = cl.getConstructor(new Class[] { ServletContext.class });
            return (ObjectNamesResourceOIDC) constr.newInstance(new Object[] { iContext });
        } catch (Exception e) {
