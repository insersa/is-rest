...
/**
 * Ressource pour accéder à un enregistrement d'un objet métier
 *
 * Les actions:
 *
 ...
 */
@Api(value = "{id}")
public class ObjectResource extends AbstractObjectResource {

    /**
     * Logger
     */
    private static final Log logger = LogFactory.getLog(ObjectResource.class);
     ...
     */
    public ObjectResource(ServletContext aContext, String aObjectName, String aId) {
    ...
    @GET
    public Response getRecord(@ApiParam(value = "Security token", required = true) @HeaderParam("token") String aToken,
        ...
        try {
            ...
            if (claims == null || iId == null || "undefined".equals(iId)) {
            ...
            json.add(Constants.TOKEN, SecurityUtil.getToken(claims, contextManager));
    ...
    public Response update(@ApiParam(value = "Security token", required = true) @HeaderParam("token") String aToken,
        ...
        try {
            ...
            // -- Contrôle si paramètre présent
            Claims claims = RestUtil.getClaims(aToken, iObjectName);
            if (claims == null || aJsonRecord == null || "undefined".equals(iId)) {
            ...
            jsonB.add(Constants.TOKEN, SecurityUtil.getToken(claims, contextManager));
            JsonObject json = jsonB.build();
            if (json.get(Constants.TOKEN) != null) {
    ...
    @PATCH
    @Consumes(MediaType.APPLICATION_JSON)
    public final Response updateFields(@HeaderParam("token") String aToken,
        ...
        try {
            ...
            IContextManager contextManager = ((ServiceLocator) iContext.getAttribute(Constants.SERVICE_LOCATOR)).getContextManager();

            // -- Contrôle si paramètre présent
            Claims claims = RestUtil.getClaims(aToken, iObjectName);
            if (claims == null || aPatch == null || "undefined".equals(iId)) {
    ...
    @DELETE
    public final Response delete(@ApiParam(value = "Security token", required = true) @HeaderParam("token") String aToken,
        ...
        try {
            ...
            // -- Contrôle si paramètre présent
            Claims claims = RestUtil.getClaims(aToken, iObjectName);
            if (claims == null || aRecord == null || "undefined".equals(iId)) {
                return Response.status(Status.BAD_REQUEST).build();
            }
            ...
            // Build the response
            JsonObject json = Json.createObjectBuilder().add("token", SecurityUtil.getToken(claims, contextManager)).build();
